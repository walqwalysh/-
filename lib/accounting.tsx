...
type ImportedJournal = { description: string; date: string; lines: Array<{ accountName: string; category: AccountCategory; debit: number; credit: number }> };
...
function normaliseLoadedData(raw: string | null): AccountingState {
  ...
  try {
    ...
    const accounts = Array.isArray(parsed.accounts) ? parsed.accounts.filter((account): account is Account => Boolean(account?.id && account?.name && categoryMeta[account.category as AccountCategory])) : [];
    const journalEntries = Array.isArray(parsed.journalEntries)
      ? parsed.journalEntries.filter((entry): entry is JournalEntry => Boolean(entry?.id && Array.isArray(entry.lines) && validateJournalLines(entry.lines).isBalanced))
      : [];
...
export function AccountingProvider({ children }: { children: ReactNode }) {
  ...
  const importJournalEntries = useCallback(async (journals: ImportedJournal[]) => {
    ...
    const lookup = new Map(next.accounts.map((account) => [`${account.category}:${account.name.trim().toLocaleLowerCase()}`, account]));
    let imported = 0; let skipped = 0;
    for (const journal of journals) {
      const mappedLines: JournalLine[] = [];
      let invalid = !journal.description.trim() || journal.lines.length < 2;
      for (const source of journal.lines) {
        const name = source.accountName.trim();
        if (!name || !categoryMeta[source.category]) { invalid = true; continue; }
        const key = `${source.category}:${name.toLocaleLowerCase()}`;
        let account = lookup.get(key);
        if (!account) { account = { id: createId("account"), name, category: source.category, createdAt: new Date().toISOString() }; next.accounts.push(account); lookup.set(key, account); }
